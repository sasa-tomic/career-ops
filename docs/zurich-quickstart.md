...
performance, and research-engineer roles. Edit `title_filter.positive` to match
the exact titles you want.
...
verified mappings, so copy any that interest you. Note that big-tech Zurich
offices (Google, Meta, Apple, Microsoft, Oracle Labs) use bespoke ATSes that
are not API-scannable; track those manually.
