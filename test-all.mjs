...
// ── 9b. JOBSPY DISCOVERY INTEGRATION ─────────────────────────────
...
if (fileExists('providers/workable.mjs')) {
  const wk = readFile('providers/workable.mjs');
  if (wk.includes("id: 'workable'") && wk.includes('/api/v1/widget/accounts/') && wk.includes('ALLOWED_WORKABLE_HOSTS')) {
    pass('workable provider wired to the widget accounts API (host-pinned)');
  } else {
    fail('workable provider missing expected widget API wiring');
  }
}
